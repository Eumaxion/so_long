...
void update_player_img(char c,t_game *data)
{
	...
	if (data->tmap.map[data->t_player.player_y][data->t_player.player_x] == 'C' && data->colect == 1)
	{
		mlx_destroy_image(data->mlx, data->tmap.img_exit);
		data->tmap.img_exit = mlx_xpm_file_to_image(data->mlx, "./assets/o_exit.xpm", &data->tmap.img_x, &data->tmap.img_y);
	}
	if (c == 'w')
...
int	handle_w(t_game *data)
{
	...
	if (data->tmap.map[data->t_player.player_y][data->t_player.player_x] == 'E' && data->colect == 0)
		...
		|| data->tmap.map[data->t_player.player_y][data->t_player.player_x] == 'E')
	...
	else
	{
		...
		data->tmap.map[data->t_player.player_y + 1][data->t_player.player_x] = '0';
		render_map(data);
	}
	return(0);
}
int	handle_a(t_game *data)
{
	...
	else
	{
		...
		if (data->tmap.map[data->t_player.player_y][data->t_player.player_x] == 'C')
			data->colect -= 1;
		data->tmap.map[data->t_player.player_y][data->t_player.player_x] = 'P';
		data->tmap.map[data->t_player.player_y][data->t_player.player_x + 1] = '0';
		render_map(data);
	}
	return(0);
}
